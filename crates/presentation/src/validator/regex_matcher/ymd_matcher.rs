pub(crate) fn ymd_matcher(text: &str) -> bool {
...
mod tests {
    ...
    /// expected: `true`
    #[test]
    fn test_ymd_matcher_valid() {
        // Check that a valid time string pass the validator
        let time_str = "2024-06-27";
        assert!(ymd_matcher(time_str));
    }

    /// Test that an invalid format is invalid
    /// we expect the format to be "%Y-%m-%d"
    /// expected: `false`
    #[test]
    fn test_ymd_matcher_invalid_format() {
    ...
    /// expected: `false`
    #[test]
    fn test_ymd_matcher_empty_string() {
        let time_str = "";
        assert!(!ymd_matcher(time_str));
    }
    ...
    /// expected: `false`
    #[test]
    fn test_ymd_matcher_partial_string() {
        let time_str = "2024-06";
        assert!(!ymd_matcher(time_str));
    }
}
