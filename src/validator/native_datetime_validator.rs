...
pub(super) fn validate_string_for_native_datetime(
    time: &str,
...
mod tests {
    ...
    fn test_accept_ym_with_hyphen_hms() {
        ...
        let result: Result<NaiveDateTime, ValidationError> =
            validate_string_for_native_datetime(time_str);
        assert!(result.is_ok());
    ...
    fn test_accept_iso_format() {
        ...
        // Check that the result value is the same as the expected
        let expected_datetime =
            NaiveDateTime::parse_from_str(time_str, "%Y-%m-%dT%H:%M:%S").unwrap();
        assert_eq!(result.unwrap(), expected_datetime);
    }
